...
Сборка `.deb` требует закрытый `pnetlab-inject.zip`, который раньше скачивался из другого GitLab project. Запустите workflow `PNETLab addon package`, передайте существующий `release_tag` и полный `inject_asset_url`. Для private URL добавьте repository secret `PNETLAB_INJECT_TOKEN`. Workflow создаст GitHub Release при необходимости и приложит `pnetlabaddon.deb`; updater уже переведён на GitHub Releases.
