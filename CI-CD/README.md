...
`PNETLab addon package` запускается вручную для существующего Git tag. Он получает полный URL закрытого `pnetlab-inject.zip`, собирает `pnetlabaddon.deb` и прикладывает его к GitHub Release. Если asset требует bearer token, добавьте repository secret `PNETLAB_INJECT_TOKEN`.
