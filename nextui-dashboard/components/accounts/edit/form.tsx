...
interface AccountsEditFormProps {
    id?: number
}
...
export const UserRoles = () => {
    const {locale: {UserForm}} = useLanguageBrowser();
    return [
...
export const AccountsEditForm = ({id}: AccountsEditFormProps) => {
    const {locale: {UserForm, Forms, Sidebar}} = useLanguageBrowser();
                ...
                <>
                    <div className='flex flex-col w-1/2 gap-4 mb-4'>
                        ...
                        <Select
                            ...
                            onSelectionChange={
                                (keys) => setFieldValue('user_role', keys.currentKey || 'student')
                            }
                        >
                            {roles.map((role) => (
                        ...
                        <Checkbox
                            type='checkbox'
                            defaultSelected={!values.deleted_at}
                            onChange={
                                handleChange('deleted_at')
                            }
                        >{UserForm.FieldIsActive}</Checkbox>
                        <Input
                            variant='bordered'
