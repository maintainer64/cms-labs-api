...
import { useMutationTargetAddonReset } from '@/helpers/queries/target/use-mutation-target-addon-reset';
...
export const TargetEditForm = ({ id }: EditFormProps) => {
  ...
  const existingRelationIds = useMemo(() => {
    ...
    return relations
      .filter((r) => r?.relation?.fromTargetId === id && r?.relation?.relationType === 'deploy')
      .map((r) => r?.relation?.toTargetId || '');
  }, [listResponse.data, id]);

  const parentTargets = useMemo(() => {
    const targets = listResponse.data?.model?.targets || [];
    return targets
      .filter((t) => t?.taget?.id !== id && !existingRelationIds.includes(t?.taget?.id || ''))
      .map((t) => ({
        key: t?.taget?.id || '',
        value: t?.taget?.name || ''
      }));
  }, [listResponse.data, id, existingRelationIds]);
  ...
  const existingRelations = useMemo(() => {
    ...
    return relations
      .filter((r) => r?.relation?.fromTargetId === id && r?.relation?.relationType === 'deploy')
      .map((r) => ({
        id: r?.relation?.toTargetId || '',
        name:
          listResponse.data?.model?.targets?.find((t) => t?.taget?.id === r?.relation?.toTargetId)?.taget?.name ||
          r?.relation?.toTargetId ||
          ''
      }));
  }, [listResponse.data, id]);
  ...
  return (
    <Formik initialValues={initialValues} validationSchema={undefined} onSubmit={handleSubmit}>
      {({ values, handleChange, handleSubmit, setFieldValue }) => (
        <>
          ...
          <Modal isOpen={isParentModalOpen} onClose={onParentModalClose}>
            <ModalContent>
              <ModalHeader>{TargetLocale.Relation?.AddParent}</ModalHeader>
              <ModalBody>
                <AutoCompleteFull
          ...
          <Tabs aria-label='Target options'>
            <Tab key='main' title={TargetLocale.Relation?.Title === 'Связи' ? 'Основное' : 'Main'}>
              <div className='flex flex-col gap-4 mb-4 p-4 border rounded-lg'>
                {isEdit && (
                  <Input variant='bordered' label={TargetLocale.FieldID} value={initialValues.id ?? ''} isReadOnly />
                )}
                <Input
                  variant='bordered'
                ...
                <div className='flex flex-wrap gap-2'>
                  ...
                  {((values.internalTags as string[]) || []).map((tag, idx) => (
                    <Chip key={`internal-tag-${idx}`} variant='flat' className='opacity-70 cursor-default'>
                      {tag}
                    </Chip>
                  ))}
                  ...
                  {!values.tags?.length &&
                    !values.links?.length &&
                    !values.internalTags?.length &&
                    !values.internalLinks?.length && (
                ...
                {existingRelations.length > 0 ? (
                  <div className='flex flex-col gap-2 mt-2'>
                    {existingRelations.map((rel) => (
                      <Chip
                        key={rel.id}
                        onClose={() => handleRemoveParent(rel.id)}
                        onClick={() => navigate(RoutesLocation.targetsEdit(rel.id))}
                        className='w-full justify-between cursor-pointer hover:bg-default-100'
                      >
                        {rel.name}
                      </Chip>
                    ))}
                  ...
                  <div className='text-gray-400 text-sm mt-2'>
                    {TargetLocale.Relation?.NoParents || 'Нет связанных'}
                  </div>
                )}
