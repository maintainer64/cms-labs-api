...
export const PnetServersList = () => {
    ...
    const {locale: {Tables: {PnetServersTable}}} = useLanguageBrowser();
    const crumbs = [
        {
            icon: <HouseIcon/>,
    ...
    const rows = response?.data?.pages.flatMap(
        (p) => p.result?.model ?? []
    ) || [];
            ...
            <>
                <div className="flex justify-between flex-wrap gap-4 items-center">
                    <div className="flex items-center gap-3 flex-nowrap w-full">
                        <SearchInput
                            placeholder={PnetServersTable.SearchBar}
                            setValue={setSearchTerm}
                        />
                        <Link to={RoutesLocation.pnetServersCreate()}>
                            <Button color="primary">
                                {PnetServersTable.ButtonAdd}
                            </Button>
                        </Link>
                    </div>
                </div>
