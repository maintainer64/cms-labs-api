...
import {
  CoreJsonRpcPath,
  transportWithAuth,
  UsecasesLTIAttemptCreateRequest,
  UsecasesLTIAttemptCreateResponse
} from '@/helpers/api';
import { TMutationCustomOptions } from '@/helpers/queries/types';
import queryClient from '@/helpers/queries/base';
import { CamelCasedPropertiesDeep } from 'type-fest';

type Params = CamelCasedPropertiesDeep<UsecasesLTIAttemptCreateRequest['params']>;
type Response = CamelCasedPropertiesDeep<UsecasesLTIAttemptCreateResponse['result']>;

export const useMutationLtiAttemptCreate = (options: TMutationCustomOptions<Response, Params> = {}) => {
  return useMutation<Response, unknown, Params>({
    // @ts-expect-error: return nullable value
    mutationFn: (params: Params) => {
      if (!params?.roomNumber) return null;
      return transportWithAuth.rpc(CoreJsonRpcPath, {
        method: 'lti_attempt.create',
        params: params
      });
    },
    ...options,
