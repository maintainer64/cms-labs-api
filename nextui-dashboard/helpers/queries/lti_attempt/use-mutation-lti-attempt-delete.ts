...
import {
  CoreJsonRpcPath,
  transportWithAuth,
  UsecasesLTIAttemptDeleteRequest,
  UsecasesLTIAttemptDeleteResponse
} from '@/helpers/api';
import { TMutationCustomOptions } from '@/helpers/queries/types';
import queryClient from '@/helpers/queries/base';
import { CamelCasedPropertiesDeep } from 'type-fest';

type Params = CamelCasedPropertiesDeep<UsecasesLTIAttemptDeleteRequest['params']>;
type Response = CamelCasedPropertiesDeep<UsecasesLTIAttemptDeleteResponse['result']>;

export const useMutationLtiAttemptDelete = (options: TMutationCustomOptions<Response, Params> = {}) => {
  return useMutation<Response, unknown, Params>({
    // @ts-expect-error: return nullable value
    mutationFn: (params: Params) => {
      if (!params?.id) return null;
      return transportWithAuth.rpc(CoreJsonRpcPath, {
        method: 'lti_attempt.delete',
        params: params
      });
    },
    ...options,
