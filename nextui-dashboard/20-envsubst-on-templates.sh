...
# Это оригинальный файл из репы 20-envsubst-on-templates.sh просто нужно в envsubst переменные из ENV_DIR тянуть
# Приходится делать так, потому что дяденьки не придумали как скриптом это нормально делать
if [ -d "$ENV_DIR" ]; then
  echo "Copy file env"
  source "$ENV_DIR/sh"
fi
