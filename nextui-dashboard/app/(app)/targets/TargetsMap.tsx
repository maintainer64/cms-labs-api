...
import { filterTargets, TargetNodeType, TargetRelationType } from '@/app/(app)/targets/target-utils';
...
function TargetMapInner({ searchQuery = '', filter = 'all' }: TargetMapProps) {
  ...
  const activeTargetIds = useMemo(() => {
    const filtered = filterTargets(data?.model?.targets || [], searchQuery, filter);
    return new Set(filtered.map((t) => t.taget.id));
  }, [isLoading, searchQuery, filter]);

  const hasActiveFilter = searchQuery.length > 0 || filter !== 'all';
  ...
  const rawData = useMemo(() => {
    ...
    targets.forEach((item) => {
      const isActive = !hasActiveFilter || activeTargetIds.has(item.taget.id);
    ...
    relations.forEach((item) => {
      ...
      const sourceActive = !hasActiveFilter || activeTargetIds.has(item.relation.fromTargetId);
      const targetActive = !hasActiveFilter || activeTargetIds.has(item.relation.toTargetId);
  ...
  }, [isLoading, activeTargetIds, hasActiveFilter]);
    ...
    <TargetWrapper>
      ...
      >
        ...
        <Panel position='top-center' className='flex gap-2 m-2'>
          <Button variant='light' size='sm' onPress={toggleDirection} className='bg-white shadow-md hover:bg-gray-50'>
            <ArrowDownUp className='h-4 w-4 mr-2' />
            {layoutDirection === 'TB' ? 'Горизонтально' : 'Вертикально'}
          </Button>
          {selectedNodeId && (
            <Button variant='light' size='sm' onPress={clearSelection} className='bg-white shadow-md hover:bg-gray-50'>
          ...
          {hasActiveFilter && (
            <div className='mt-2 pt-2 border-t border-gray-200'>
              <div className='flex items-center gap-2'>
                ...
                <span className='text-xs text-gray-500'>Не в фильтре</span>
              </div>
            </div>
          )}
        </Panel>
