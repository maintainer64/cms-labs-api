...
func (u *SSOSecondFactorUC) Execute(dto SSOSecondFactorInputDTO) (*SSOSecondFactorOutputDTO, error) {
	...
	u.Logger.Info().Msg(
		fmt.Sprintf(
			"Get token by OTP code user_id=%s, state=%s", token.UserId, token.State,
		),
	)
	...
	u.Logger.Info().Msg(
		fmt.Sprintf(
			"Get user by access token user_id=%s, email=%s", user.Sub, user.Email,
		),
	)
	if user.Email == "" {
	...
	if err != nil {
		u.Logger.Info().Msg(
			fmt.Sprintf(
				"Get or create user user_id=%d, email=%s with error=%+v", user.UserID(), user.Email, err,
			),
		)
		return nil, err
	}
	u.Logger.Info().Msg(
		fmt.Sprintf(
			"Get or create user user_id=%d, email=%s user_pod=%d", user.UserID(), user.Email, userDB.Pod,
		),
	)
	userDB.Name = user.Sub
	...
	u.Logger.Info().Msg(
		fmt.Sprintf(
			"Response cookies generated by user_id=%s, email=%s", user.Sub, user.Email,
		),
	)
	//
