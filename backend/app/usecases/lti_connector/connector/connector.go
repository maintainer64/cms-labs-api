...
func sendRequest(req *http.Request, log *zerolog.Logger) (datastore.AccessToken, error) {
	...
	log.Debug().Msg(
		fmt.Sprintf(
			"lti/sendRequest to create access token %+v",
			req,
		),
	)
	log.Info().Msg(fmt.Sprintf("lti/sendRequest to create access token [%s] %s", req.Method, req.URL))
	response, err := client.Do(req)
	if err != nil {
		return datastore.AccessToken{}, fmt.Errorf("send request error: %w", err)
	}
	log.Info().Msg(
		fmt.Sprintf("lti/sendRequest to create access token response statusCode: %+v",
			response.StatusCode,
		),
	)
...
func (c *Connector) GetAccessToken(scopes []string) error {
	...
	_ = c.cfg.AccessTokens.StoreAccessToken(responseToken)
