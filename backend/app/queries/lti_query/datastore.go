...
// Package lti_query implements the interfaces and types for all the different storers used in LTI.
package lti_query
...
type LaunchDataStorer interface {
	...
	StoreLaunchData(launchID string, launchData json.RawMessage, regUid uint) error
...
// An UserInfoStorer manages the storage of user model
type UserInfoStorer interface {
	// UpdateByLaunchData save json data.
	UpdateByLaunchData(launchID string, launchData json.RawMessage) error
}
